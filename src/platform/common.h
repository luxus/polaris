...
#undef _CONVERT
  ...
  inline std::string_view from_frame_transport(frame_transport_e transport) {
#define _CONVERT(x) \
  case frame_transport_e::x: \
    return #x##sv
    switch (transport) {
      _CONVERT(shm);
      _CONVERT(dmabuf);
      _CONVERT(dxgi_duplication);
      _CONVERT(wgc);
      _CONVERT(d3d11_shared);
      _CONVERT(internal);
      _CONVERT(unknown);
    }
#undef _CONVERT
  ...
  enum class frame_residency_e {
    cpu,
    gpu,
    unknown,
  };

  inline std::string_view from_frame_residency(frame_residency_e residency) {
#define _CONVERT(x) \
  case frame_residency_e::x: \
    return #x##sv
    switch (residency) {
      _CONVERT(cpu);
      _CONVERT(gpu);
...
#undef _CONVERT
  ...
  inline std::string_view from_frame_format(frame_format_e format) {
#define _CONVERT(x) \
  case frame_format_e::x: \
    return #x##sv
    switch (format) {
      _CONVERT(bgra8);
      _CONVERT(rgba16f);
...
#undef _CONVERT
  ...
  public:
    ...
    mem_type_e native_device_type {mem_type_e::unknown};
  ...
  struct encode_device_t {
    ...
    virtual mem_type_e native_output_device_type() const {
      return mem_type_e::unknown;
    }

    virtual void *native_output_resource() const {
      return nullptr;
    }

    virtual int native_output_fd() const {
      return -1;
    }
