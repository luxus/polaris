...
#ifdef __GNUC__
  #define POLARIS_GNUC_EXTENSION __extension__
#else
  #define POLARIS_GNUC_EXTENSION
#endif
...
namespace dyn {
  ...
  int load(void *handle, const std::vector<std::tuple<apiproc *, const char *>> &funcs, bool strict) {
    ...
    for (auto &func : funcs) {
      ...
      *fn = POLARIS_GNUC_EXTENSION(apiproc) dlsym(handle, name);
  ...
  fs::path appdata() {
    ...
    std::call_once(migration_flag, []() {
      ...
      if ((dir = getenv("CONFIGURATION_DIRECTORY")) != nullptr && strlen(dir) > 0) {
        found = true;
        config_path = fs::path(dir) / "polaris"sv;
      }
...
std::string get_local_ip_for_gateway() {
  ...
  void adjust_thread_priority(thread_priority_e priority) {
    ...
    if (priority >= thread_priority_e::high) {
      ...
      param.sched_priority = (priority == thread_priority_e::critical) ? 30 : 20;

      if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &param) == 0) {
        BOOST_LOG(debug) << "Set thread to SCHED_FIFO priority "sv << param.sched_priority;
        return;
      }
  ...
  std::unique_ptr<deinit_t> init() {
    ...
    if ((config::video.capture.empty() && sources.none()) || config::video.capture == "wlr"
      ...
      // compositor may still fall back to SHM depending on renderer support.
      if (config::video.linux_display.use_cage_compositor) {
        if (config::video.linux_display.headless_mode) {
          BOOST_LOG(info) << "Cage compositor configured in headless mode — will use direct wlr capture (frame transport determined at runtime)"sv;
        } else {
          BOOST_LOG(info) << "Cage compositor configured — will use direct wlr capture with DMA-BUF when available"sv;
        }
    ...
    if ((config::video.capture.empty() && sources.none()) || config::video.capture == "portal") {
      ...
      if (verify_portal()) {
        ...
        BOOST_LOG(info) << "Portal capture works on GNOME, KDE, and wlroots without elevated privileges."sv;
      }
    }
#endif
#ifdef POLARIS_BUILD_X11
