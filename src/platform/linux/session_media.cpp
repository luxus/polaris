...
namespace session_media {
  ...
  void prepare_for_stop() {
    ...
    if (!g_prepare_inflight.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
      BOOST_LOG(info) << "session_media: prepare_for_stop already in flight; waiting"sv;
      // Spin briefly for the in-flight prepare to finish rather than nesting
      // a second portal release.
      for (int i = 0; i < 100 && g_prepare_inflight.load(std::memory_order_acquire); ++i) {
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
      }
      if (g_prepare_inflight.load(std::memory_order_acquire)) {
        BOOST_LOG(warning) << "session_media: previous prepare still busy; skipping nested prepare"sv;
      }
      return;
    }
  ...
  bool teardown_busy() {
    if (g_busy.load(std::memory_order_acquire) ||
        g_prepare_inflight.load(std::memory_order_acquire)) {
      return true;
    }
    std::lock_guard lock(g_worker_mu);
    return !g_queue.empty();
