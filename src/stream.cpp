...
#pragma pack(pop)
   ...
   */
  /**
   * @brief Combines two buffers and inserts new buffers at each slice boundary, writing into a reusable output buffer.
   * @param result The reusable output buffer (will be resized, not reallocated if capacity suffices).
   * @param insert_size The number of bytes to insert.
   * @param slice_size The number of bytes between insertions.
   * @param data1 The first data buffer.
   * @param data2 The second data buffer.
  ...
  void concat_and_insert_into(std::vector<uint8_t> &result, uint64_t insert_size, uint64_t slice_size, const std::string_view &data1, const std::string_view &data2) {
    ...
    for (auto x = 0; x < elements; ++x) {
      ...
      if (next + slice_size > end) {
        ...
        std::memcpy((char *) p + insert_size, next, copy_len);

        // Copy the remaining portion from the second buffer
        next = std::begin(data2);
        end = std::end(data2);
        std::memcpy((char *) p + copy_len + insert_size, next, slice_size - copy_len);
        next += slice_size - copy_len;
      } else {
        std::memcpy((char *) p + insert_size, next, slice_size);
        next += slice_size;
      }
    }
  }
  ...
  /**
   * @brief Replaces the first occurrence of a pattern in the source, writing into a reusable output buffer.
   * @param result The reusable output buffer (will be resized, not reallocated if capacity suffices).
   * @param original The source data.
   * @param old The pattern to find.
   * @param _new The replacement pattern.
   */
  void replace_into(std::vector<uint8_t> &result, const std::string_view &original, const std::string_view &old, const std::string_view &_new) {
    auto begin = std::begin(original);
    auto end = std::end(original);
    auto match = std::search(begin, end, std::begin(old), std::end(old));

    if (match == end) {
      // No match — copy original as-is
      result.resize(original.size());
      std::memcpy(result.data(), original.data(), original.size());
      return;
    }

    auto prefix_len = match - begin;
    auto suffix_start = match + old.size();
    auto suffix_len = end - suffix_start;

    result.resize(prefix_len + _new.size() + suffix_len);
    std::memcpy(result.data(), begin, prefix_len);
    std::memcpy(result.data() + prefix_len, _new.data(), _new.size());
    std::memcpy(result.data() + prefix_len + _new.size(), suffix_start, suffix_len);
  }
  ...
  void videoBroadcastThread(udp::socket &sock) {
    ...
    // Persistent reusable buffers — avoids per-frame heap allocation.
    // These grow to steady-state size within the first few frames and stay there.
    std::vector<uint8_t> replace_bufs[2];  // ping-pong for multiple replacements
    std::vector<uint8_t> concat_buf;
    ...
    while (auto packet = packets->pop()) {
      ...
      if (packet->is_idr() && packet->replacements) {
        ...
        for (auto &replacement : *packet->replacements) {
          ...
          payload = {(char *) buf.data(), buf.size()};
          replace_idx ^= 1;  // alternate buffers to avoid source/dest overlap
        }
      }
      ...
      concat_and_insert_into(concat_buf, sizeof(video_packet_raw_t), payload_blocksize, std::string_view {(char *) &frame_header, sizeof(frame_header)}, payload);
  ...
  int start_broadcast(broadcast_ctx_t &ctx) {
    ...
    // Set audio socket send buffer size (SO_SENDBUF) to 512KB
    try {
      ctx.audio_sock.set_option(boost::asio::socket_base::send_buffer_size(512 * 1024));
    } catch (...) {
  ...
  namespace session {
    ...
    int start(session_t &session, const std::string &addr_string) {
      ...
      if (max_sessions > 0) {
        auto current_count = running_sessions.load(std::memory_order_relaxed);
        if (static_cast<int>(current_count) >= max_sessions) {
          BOOST_LOG(warning) << "Rejecting new session from "sv << addr_string
                             << ": max_sessions limit reached ("sv << current_count << "/"sv << max_sessions << ")"sv;
          return -1;
        }
      }
