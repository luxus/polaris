...
namespace audio {
  ...
  void encodeThread(sample_queue_t samples, config_t config, void *channel_data, packet_queue_t packets) {
    ...
    // Enable Opus DRED (Deep Redundancy) for packet loss resilience.
    // Allows the decoder to reconstruct up to 100ms of lost audio without retransmission.
    // Requires libopus >= 1.5.0. Adds ~5% bitrate overhead but dramatically improves
    // audio quality on lossy WiFi connections.
#ifdef OPUS_SET_DRED_DURATION_REQUEST
    auto dred_err = opus_multistream_encoder_ctl(opus.get(), OPUS_SET_DRED_DURATION(100));
    if (dred_err == OPUS_OK) {
      BOOST_LOG(info) << "Opus DRED enabled (100ms redundancy)";
    } else {
      BOOST_LOG(debug) << "Opus DRED not available: " << opus_strerror(dred_err);
    }
#endif
